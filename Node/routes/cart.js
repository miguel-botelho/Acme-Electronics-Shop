		...
		if (cart.idUser == req.params.idUser) {
			if (cart.idProduct == req.params.idProduct) {
				...
				stmt.get([cart.quantity + 1, cart.idProduct, cart.idCart], (err, result) => {
					console.log(result);
					res.send('Success adding another product of the same type');
				});
			} else {
				...
				stmt.get([1, req.params.idProduct, cart.idCart], (err, result) => {
					console.log(result);
					res.send('Success adding a different product');
				});
			}
		} else {
			console.log(cart);
		...
		if (cart.idUser == req.params.idUser) {
			if (cart.idProduct == req.params.idProduct) {
				...
				stmt.get([cart.idProduct, cart.idCart], (err, result) => {
					console.log(result);
					res.send('Success removing product');
				});
			} else {
				console.log(cart);
				res.send('That product doesn\'t exist in the cart');
			}
		} else {
			console.log(cart);
			res.send('Error');
